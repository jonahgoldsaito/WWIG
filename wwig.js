  ...
  jQuery(document).ready(function($) {
    ...
    $.getJSON( "http://api.twitter.com/1/statuses/user_timeline.json?callback=?",
		  ...
		  function(data) {
		    ...
		    if ( $tpd.length==0 ){
          $tpd = $('<li id="wwig-tweets-per-day" class="new_stats"><strong>'+tweets_per_day+'</strong> Tweets / Day</li>')
                  .appendTo(('.js-mini-profile-stats')).hide().slideDown();
          $wpd = $('<li id="wwig-wwig-per-day" class="new_stats"><strong>'+wwig_per_day+'</strong> WWIG / Day</li>')
                  .appendTo(('.js-mini-profile-stats')).hide().slideDown();
